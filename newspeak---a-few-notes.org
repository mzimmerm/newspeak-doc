...
* IN-PROGRESS-NOW HopscotchForHTML5: Analysing it by digging though code; discovering Subject, Model, Presenter and Fragment; writing a UI app based on what we learned
...
todo-later - link to example when ready
...
**** IN-PROGRESS-NOW Add the packaging of the HopscotchForHTML5 module
...
While we keep the full code listed here, in the IDE we find the ~CheckedItemApp>>#packageUsing: manifest~ method and paste the 3-line change there.

todo-later add screenshot.
...
The expression defining the ~platformWithHopscotch~ is perhaps the only thing we did not fully explain. We did explain the "true" branch but not the "false" branch. There is a detail description in the final code.
...
todo-later - add link to concrete UI code.
...
From this summary, our ~CheckedItemPresenter~ (which will be, as it's model and subject, a nested class in CheckedItemUI) should look like this:
...
todo-later add section on how basic components : checkbox, redio, label, button handle actions


As with the model and presenter, an inner class of CheckedItemUI is added by clicking the + beside Classes inside CheckedItemUI:
...
So let us use this snippet into our code - the only difference is the class names,  instead of starting with /Concrete/ starts with /CheckedItem/. We are making this code unnecessarily verbatim and long, to spell out what we discussed:
...
todo links to newspeak-docs

When running, this is how it looks:
